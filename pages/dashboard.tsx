...
export default function dashboard ({data, buys, id, token}) {
...
export async function getServerSideProps(ctx) {
    ...
    const res = await fetch(`http://localhost:7000/api/v2/spendingApp/users/${id}`, { headers: {'Content-type' : 'application/json','x-auth-token': token})
    const purch = await fetch(`http://localhost:7000/api/v2/spendingApp/purchase?user=${id}`, {
            headers: {
